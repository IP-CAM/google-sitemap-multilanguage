...
class ControllerFeedGoogleSitemapMl extends Controller {
	...
	public function index() {
		$data = $this->language->load('feed/google_sitemap_ml');
		...
		$data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('feed/google_sitemap_ml', 'token=' . $this->session->data['token'], 'SSL'),
			'separator' => ' :: '
		);

		$data['action'] = $this->url->link('feed/google_sitemap_ml', 'token=' . $this->session->data['token'], 'SSL');
		...
		$this->response->setOutput($this->load->view('feed/google_sitemap_ml.tpl', $data));
	} 

	protected function validate() {
		if (!$this->user->hasPermission('modify', 'feed/google_sitemap_ml')) {
